...
local function get_gcd(nodes)
    ...
    --ngx.log(ngx.ERR, "first_id"..first_id)
    --ngx.log(ngx.ERR, "max_weight"..max_weight)
...
function _M.new(_, nodes)
    ...
    --ngx.log(ngx.ERR, "newnodes max_weight:" , max_weight , " gcd:", gcd, " nodes:", json.encode(newnodes))
    --ngx.log(ngx.ERR, "newnodes only_key:" , only_key)
    --ngx.log(ngx.ERR, "newnodes gcd:" .. gcd)
    --ngx.log(ngx.ERR, "newnodes max_weight:" .. max_weight)
...
local function find(self)
    ...
    if only_key then
        --ngx.log(ngx.ERR, "find only_key".. only_key)
        return only_key
    end
    ...
    while true do
        while true do
            ...
            if weight >= cw then
                ...
                --ngx.log(ngx.ERR, "find last_id:".. last_id .. " cw:" .. cw .. "weight:" .. weight)
                return last_id
            end
        end
